...
static UniValue gobject_prepare(const JSONRPCRequest& request)
{
    ...
    // This command is dangerous because it consumes 5 OSMIUM irreversibly.
    // If params are lost, it's very hard to bruteforce them and yet
    // users ignore all instructions on osmiumcentral etc. and do not save them...
...
{ //  category              name                      actor (function)         argNames
  //  --------------------- ------------------------  -----------------------  ----------
    /* Osmium features */
    { "osmium",               "getgovernanceinfo",      &getgovernanceinfo,      {} },
    { "osmium",               "getsuperblockbudget",    &getsuperblockbudget,    {"index"} },
    { "osmium",               "gobject",                &gobject,                {} },
    { "osmium",               "voteraw",                &voteraw,                {"tx_hash","tx_index","gov_hash","signal","outcome","time","sig"} },
