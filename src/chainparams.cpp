...
public:
    CMainParams() {
        ...
        vSeeds.emplace_back("dnsseed.osmium.org");

        // Osmium addresses start with 'X'
        base58Prefixes[PUBKEY_ADDRESS] = std::vector<unsigned char>(1,76);
        // Osmium script addresses start with '7'
        base58Prefixes[SCRIPT_ADDRESS] = std::vector<unsigned char>(1,16);
        // Osmium private keys start with '7' or 'X'
...
public:
    CTestNetParams() {
        ...
        vSeeds.emplace_back("testnet-seed.osmiumdot.io"); // Just a static list of stable node(s), only supports x9

        // Testnet Osmium addresses start with 'y'
        base58Prefixes[PUBKEY_ADDRESS] = std::vector<unsigned char>(1,140);
        // Testnet Osmium script addresses start with '8' or '9'
...
public:
    explicit CDevNetParams(const ArgsManager& args) {
        ...
        vFixedSeeds.clear();
        vSeeds.clear();
        //vSeeds.push_back(CDNSSeedData("osmiumevo.org",  "devnet-seed.osmiumevo.org"));
...
public:
    explicit CRegTestParams(const ArgsManager& args) {
        ...
        // Regtest Osmium addresses start with 'y'
        base58Prefixes[PUBKEY_ADDRESS] = std::vector<unsigned char>(1,140);
        // Regtest Osmium script addresses start with '8' or '9'
        base58Prefixes[SCRIPT_ADDRESS] = std::vector<unsigned char>(1,19);
