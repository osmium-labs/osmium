...
BOOST_FIXTURE_TEST_CASE(util_ArgsMerge, ArgsMergeTestingSetup)
{
    ...
    //
    //   ARGS_MERGE_TEST_OUT=results.txt ./test_osmium --run_test=util_tests/util_ArgsMerge
    //
...
BOOST_FIXTURE_TEST_CASE(util_ChainMerge, ChainMergeTestingSetup)
{
    ...
    // If check below fails, should manually dump the results with:
    //
    //   CHAIN_MERGE_TEST_OUT=results.txt ./test_osmium --run_test=util_tests/util_ChainMerge
    //
    // And verify diff against previous results to make sure the changes are expected.
    //
    // Results file is formatted like:
...
BOOST_AUTO_TEST_CASE(util_ParseMoney)
{
    ...
    // Osmium's supply cap is MAX_MONEY = 1,210,000, so 10,000,000 is not representable here
    // (upstream assumed a 21,000,000 cap). Pin the boundary instead: the cap itself parses,
    // one satoshi above it does not.
    BOOST_CHECK_EQUAL(ParseMoney("1210000.00").value(), MAX_MONEY);
    BOOST_CHECK(!ParseMoney("1210000.00000001"));
...
BOOST_AUTO_TEST_CASE(message_sign)
{
    const std::array<unsigned char, 32> privkey_bytes = {
        ...
        // derived address from this private key: SRVRz6R8ABTkrowPfimNUPnUNmrks2EsJv
...
BOOST_AUTO_TEST_CASE(message_verify)
{
    ...
    BOOST_CHECK_EQUAL(
        MessageVerify(
            "7K5nz1wwa1XiPGifsSvobRKRvYBxwYSUfn",
            "signature should be irrelevant",
            "message too"),
    ...
    BOOST_CHECK_EQUAL(
        MessageVerify(
            "SRPeJi3PB42UK7A5GbPtMaQSk8FUutrsS5",
