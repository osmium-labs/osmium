...
BOOST_AUTO_TEST_CASE(basic_transaction_tests)
{
    ...
    stream >> tx;
    TxValidationState state;
    BOOST_CHECK_MESSAGE(CheckTransaction(CTransaction(tx), state, 0, 0) && state.IsValid(), "Simple deserialized transaction should be valid.");

    // Check that duplicate txins fail
    tx.vin.push_back(tx.vin[0]);
    BOOST_CHECK_MESSAGE(!CheckTransaction(CTransaction(tx), state, 0, 0) || !state.IsValid(), "Transaction with duplicate txins should be invalid.");
}
