...
void RPCConsole::updateMasternodeCount()
{
    ...
    size_t total_super_count = mnList.GetAllSuperCount();
    size_t total_enabled_super_count = mnList.GetValidSuperCount();
    QString strMasternodeCount = tr("Total: %1 (Enabled: %2)")
        .arg(QString::number(total_mn_count - total_super_count))
        .arg(QString::number(total_enabled_mn_count - total_enabled_super_count));
    ui->masternodeCount->setText(strMasternodeCount);
    QString strSuperCount = tr("Total: %1 (Enabled: %2)")
            .arg(QString::number(total_super_count))
            .arg(QString::number(total_enabled_super_count));
    ui->superCount->setText(strSuperCount);
}
