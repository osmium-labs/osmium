...
- [Development guidelines](#development-guidelines)
    - [General Osmium Core](#general-osmium-core)
    - [Wallet](#wallet)
    - [General C++](#general-c)
...
Osmium Core uses [Doxygen](https://www.doxygen.nl/) to generate its official documentation.
...
Osmium Core is a multi-threaded application, and deadlocks or other
...
([`valgrind.supp`](https://github.com/osmium-labs/osmium/blob/master/contrib/valgrind.supp))
which includes known Valgrind warnings in our dependencies that cannot be fixed
in-tree. Example use:

```shell
$ valgrind --suppressions=contrib/valgrind.supp src/test/test_osmium
$ valgrind --suppressions=contrib/valgrind.supp --leak-check=full \
      --show-leak-kinds=all src/test/test_osmium --log_level=test_suite
$ valgrind -v --leak-check=full src/osmiumd -printtoconsole
...
# A coverage report will now be accessible at `./test_osmium.coverage/index.html`.
```
...
To profile a running osmiumd process for 60 seconds, you could use an
invocation of `perf record` like this:

```sh
$ perf record \
    -g --call-graph dwarf --per-thread -F 140 \
    -p `pgrep osmiumd` -- sleep 60
```
...
Osmium Core can be compiled with various "sanitizers" enabled, which add
...
- [Main thread (`osmiumd`)](https://doxygen.bitcoincore.org/bitcoind_8cpp.html#a0ddf1224851353fc92bfbff6f499fa97)
...
However, in open source software such as Osmium Core, where everyone uses
...
pay attention to for reviewers of Osmium Core code.
...
- For `strprintf`, `LogPrint`, `LogPrintf` formatting characters don't need size specifiers.
  ...
  - *Rationale*: Osmium Core uses tinyformat, which is type safe. Leave them out to avoid confusion.
...
be sent upstream, but bugfixes may also be prudent to PR against Osmium Core so that they can be integrated
...
- Try not to overload methods on argument type. E.g. don't make `getblock(true)` and `getblock("hash")`
  ...
  - *Rationale*: This is impossible to use with `osmium-cli`, and can be surprising to users.
...
- Add every non-string RPC argument `(method, idx, name)` to the table `vRPCConvertParams` in `rpc/client.cpp`.
  ...
  - *Rationale*: `osmium-cli` and the GUI debug console use this table to determine how to
...
- Wallet RPCs call BlockUntilSyncedToCurrentChain to maintain consistency with
  ...
  - *Rationale*: In previous versions of Osmium Core, the wallet was always
...
- Use *invalid* Osmium addresses (e.g. in the constant array `EXAMPLE_ADDRESS`) for
