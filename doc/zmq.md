...
The Osmium Core daemon can be configured to act as a trusted "border
router", implementing the osmium wire protocol and relay, making
...
The ZeroMQ feature in Osmium Core requires the ZeroMQ API >= 4.0.0
...
during the *configure* step of building osmiumd:
...
For instance:
    ...
    $ osmiumd -zmqpubhashtx=tcp://127.0.0.1:28332 \
               -zmqpubhashtx=tcp://192.168.1.2:28332 \
               -zmqpubrawtx=ipc:///tmp/osmiumd.tx.raw \
...
These options can also be provided in osmium.conf.
...
From the perspective of osmiumd, the ZeroMQ socket is write-only; PUB
sockets don't even have a read function. Thus, there is no state
introduced into osmiumd directly. Furthermore, no information is
...
using. Osmiumd appends an up-counting sequence number to each
