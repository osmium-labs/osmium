...
git clone https://github.com/osmium/osmium.git
```
...
_Note: this step can be skipped if [our CI](https://github.com/osmium-labs/osmium/blob/master/ci/test/00_setup_env.sh#L64) still uses bitcoin's SDK package (see SDK_URL)_
...
* [ ] Update the osmium.org download links
* [ ] Ensure that docker hub images are up to date

### Announce the release:
* [ ] Release on Osmium forum: https://www.osmium.org/forum/topic/official-announcements.54/ (necessary so we have a permalink to use on twitter, reddit, etc.)
...
* [ ] Update Core and User docs (docs.osmium.org)
* [ ] Test Docker build runs without error in Osmiummate
...
```sh
xcrun notarytool submit osmiumcore-{version}-{x86_64, arm64}-apple-darwin.dmg --keychain-profile "AC_PASSWORD" --wait
```
